...
var rawCalibrations = @"xt36five77
two8five6zfrtjj
...
var rawCalibrations2 = @"xt36five77
two8five6zfrtjj
eightthree8fiveqjgsdzgnnineight
7chmvlhnpfive
1tcrgthmeight5mssseight
eightoneqxspfzjk4zpfour
fdbtmkhdfzrck9kxckbnft
...
var calibrationsToUse = rawCalibrations2;

var calibrations = calibrationsToUse.Split("\r\n");

var sum = 0;
foreach(var calibration in calibrations)
{
    var calibrationWithSub = calibration.ReplaceNumberStrs();
    var (digit1, index1) = GetFirstDigit(calibrationWithSub);
    var (digit2, index2) = GetFirstDigit(calibrationWithSub.Reverse());
    index2 = calibrationWithSub.Length - index2 - 1;
    if (digit1 < 0 || digit2 < 0) continue;
    if (index1 == index2) continue;
    Console.WriteLine($"{digit1}{digit2} {index1} {index2} {calibration} {calibrationWithSub}");
    sum += (digit1 * 10) + digit2;
}
Console.WriteLine(sum);

(int, int) GetFirstDigit(IEnumerable<char> str)
{
    var counter = -1;
    foreach (var char_ in str)
    {
        counter++;
        var val = char_.CharSwitch();
        if (val < 0) continue;
        return (val, counter);
    }
    return (-1, counter);
